-- CREDIT TO Kefta FOR THE BASE OF THE WEAPON SELECTOR (SOME TWEAKS MADE BUT THANKS REGARDLESS) : https://raw.githubusercontent.com/Kefta/Weapon-Switcher-Skeleton/master/gs_switcher.lua
...
ResChange()
hook.Add("OnScreenSizeChanged", "GubHUDWeaponSel", ResChange)

local function DrawWeaponHUD()
    draw.RoundedBox(0, x, y, width, height, theme.secondarycolor)
	draw.RoundedBox(0, x + 3, y + 3, width - 6, height - 6, theme.primarycolor)
    draw.SimpleTextOutlined("Slot " .. iCurSlot, "gubfont_primary_20", center, y, theme.white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 0, Color(0, 0, 0))

    local tSlot = tCache[iCurSlot]
	local iSlotWeapons = tCacheLength[iCurSlot]
	local pSelectedWeapon = tCache[iCurPos]
	local pPlayer = LocalPlayer()
    local colDraw = theme.white
    ...
    for i = 1, iSlotWeapons do
        ...
        if (pWeapon:IsValid()) then
            iCurHeight = iCurHeight + height + 5
            //draw.RoundedBoxEx(10, x, iCurHeight, width, height, theme.primarycolor, false, false, false, false)

			draw.RoundedBox(0, x, iCurHeight, width, height, iCurPos == i and theme.secondarycolor or theme.primarycolor)
			draw.RoundedBox(0, x + 3, iCurHeight + 3, width - 6, height - 6, theme.primarycolor)

            draw.SimpleTextOutlined(pWeapon:GetPrintName(), "gubfont_primary_20", center, iCurHeight, iCurPos == i and theme.secondarycolor or theme.white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 0, Color(0, 0, 0))
        end
    end
...
hook_Add("PlayerBindPress", "GS_WeaponSelector", function(pPlayer, sBind, bPressed)
	...
	-- Last weapon switch
	if (sBind == "lastinv") and not pPlayer:KeyDown(IN_ATTACK) and not pPlayer:KeyDown(IN_USE) then
		if (bPressed) then
	...
	-- Close the menu
	if (sBind == "cancelselect") and not pPlayer8:KeyDown(IN_ATTACK) and not pPlayer:KeyDown(IN_USE) then
	...
	if (sBind == "invprev") and not pPlayer:KeyDown(IN_ATTACK) and not pPlayer:KeyDown(IN_USE) then
	...
	if (sBind == "invnext") and not pPlayer:KeyDown(IN_ATTACK) and not pPlayer:KeyDown(IN_USE) then
		if (not bPressed) then
			return true
		end
	...
	-- Keys 1-6
	if (string_sub(sBind, 1, 4) == "slot") and not pPlayer:KeyDown(IN_ATTACK) and not pPlayer:KeyDown(IN_USE) then
